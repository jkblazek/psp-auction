...
def run_simulation(num_buyers = 10, num_sellers = 5):
...
def run_simulation_with_damping(use_damping, num_iterations=20, influence_threshold=50.0, num_buyers = 100, num_sellers = 35):
    """
    Run the simulation with or without damping factor.
    
    :param use_damping: If True, use damping factor; otherwise, set it to 1.0 for all nodes.
    :param num_iterations: Number of iterations to run the simulation.
    :param influence_threshold: Threshold for damping calculation.
    :return: Recorded prices and bids over iterations.
    """
    # Create test setup
    sellers = [
        Node(f"Seller_{i}", price=np.random.uniform(50, 100), quantity=-np.random.uniform(10, 20))
        for i in range(num_sellers)
    ]
    buyers = [
        Node(f"Buyer_{i}", price=0, quantity=np.random.uniform(10, 20))
    ...
    price_history = {seller.id: [] for seller in sellers}
    bid_history = {buyer.id: [] for buyer in buyers}

    for iteration in range(num_iterations):
        damping_factors = {}
        for node_id in G_seller_buyer.nodes:
            if use_damping:
                damping_factors[node_id] = influence_function(
                    node_id, G_seller_buyer, G_buyer_buyer, G_seller_seller, influence_threshold
                )
            else:
                damping_factors[node_id] = 1.0  # No damping effect

        # Update influence sets (only modifies graph weights for debugging)
        calculate_influence_sets_with_damping(G_seller_buyer, damping_factors)

        # Update bids and prices
        for node_id in G_seller_buyer.nodes:
            node = G_seller_buyer.nodes[node_id]['obj']
            damping_factor = damping_factors[node_id]
            ...
            if "Seller" in node_id:
                ...
                bid_history[node_id].append(node.bid)

    return price_history, bid_history

def compare_convergence():
    """
    Compare the convergence of the auction mechanism with and without the damping factor.
    """
    num_iterations = 20

    # Run simulations
    print("Running simulation with damping factor...")
    prices_with_damping, bids_with_damping = run_simulation_with_damping(use_damping=True, num_iterations=num_iterations)
    print("Running simulation without damping factor...")
    prices_without_damping, bids_without_damping = run_simulation_with_damping(use_damping=False, num_iterations=num_iterations)

    # Plot convergence for sellers
    plt.figure(figsize=(12, 6))
    for seller_id in prices_with_damping:
        plt.plot(prices_with_damping[seller_id], label=f"{seller_id} (with damping)", linestyle='-')
        plt.plot(prices_without_damping[seller_id], label=f"{seller_id} (no damping)", linestyle='--')

    plt.title("Seller Price Convergence with and without Damping")
    plt.xlabel("Iterations")
    plt.ylabel("Price")
    plt.legend()
    plt.show()

    # Plot convergence for buyers
    plt.figure(figsize=(12, 6))
    for buyer_id in bids_with_damping:
        plt.plot(bids_with_damping[buyer_id], label=f"{buyer_id} (with damping)", linestyle='-')
        plt.plot(bids_without_damping[buyer_id], label=f"{buyer_id} (no damping)", linestyle='--')

    plt.title("Buyer Bid Convergence with and without Damping")
    plt.xlabel("Iterations")
    plt.ylabel("Bid")
    plt.legend()
    plt.show()
